...
class BetterSqlite3 implements Driver {
  ...
  var doOpen:String->BetterSqlite3Connection;

  public function new(?fileForName:String->String) 
    this.doOpen = function (name) {
      if (fileForName != null) name = fileForName(name);
      return sys.db.Sqlite.open(name);
    }
  
  public function open<Db:DatabaseInfo>(name:String, info:Db):Connection<Db> {
    var cnx = doOpen(name);
    return new BetterSqlite3Connection(info, cnx, new SqlFormatter());
  }
    doOpen(name);
...
private extern class NativeDriver {
  function new(): Void;
  static function prepare(query:String):Statement;
}

private extern class Statement {
  function get(): js.JsIterator;
}

class BetterSqlite3Connection<Db:DatabaseInfo> implements Connection<Db> implements Sanitizer {
